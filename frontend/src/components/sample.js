...
class Sample extends React.Component {
    ...
    render() {
        return (
            <div>
                <p>{text}</p>
                {Items(this, this.props.state.data)}
                {ItemCreator(this)}
            </div>
        );
    }
...
const text =
    'This is a sample component. It lists the items from route GET /items';
...
const ItemEditor = (self, item) => (
    <span>
        ...
        <button onClick={self.cancelEdit}>Cancel</button>
    </span>
);

const DeleteButton = (self, item) => (
    <button onClick={() => self.deleteItem(item)}>Delete</button>
);
